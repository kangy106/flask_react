...
def getSleepForm(data):
    data.id = int(request.form["id"])
    num = data.numberTimes
    data.numberTimes = num + 1
    data.getupHour = int(request.form["getupHour"])
    data.getupMin = int(request.form["getupMin"])
    data.sleepHourStart  = int(request.form["sleepHourStart"])
    data.sleepMinStart = int(request.form["sleepMinStart"])            
    data.sleepHourEnd  = int(request.form["sleepHourEnd"])            
    data.sleepMinEnd = int(request.form["sleepMinEnd"])
    data.goingoutHourPre = int(request.form["goingoutHourPre"])
    data.goingoutMinPre = int(request.form["goingoutMinPre"])
    data.goingoutHour = int(request.form["goingoutHour"])
    data.goingoutMin = int(request.form["goingoutMin"])                       
    data.cookingHourPre  = int(request.form["cookingHourPre"])
    data.cookingMinPre  = int(request.form["cookingMinPre"])
    data.cookingHour  = int(request.form["cookingHour"])
    data.cookingMin  = int(request.form["cookingMin"])
    data.eatingHourPre = int(request.form["eatingHourPre"])            
    data.eatingMinPre = int(request.form["eatingMinPre"])            
    data.eatingHour = int(request.form["eatingHour"])            
    data.eatingMin = int(request.form["eatingMin"])            
    data.bathingHourPre  = int(request.form["bathingHourPre"])            
    data.bathingMinPre  = int(request.form["bathingMinPre"])            
    data.bathingHour  = int(request.form["bathingHour"])            
    data.bathingMin  = int(request.form["bathingMin"])            
    ...
    print(data.getupHour+data.getupMin/60)
    print(dtNow.hour+dtNow.minute/60)
    print(dtNow.hour+dtNow.minute/60-data.getupHour-data.getupMin/60-data.goingoutLengthPre-data.cookingLengthPre-data.eatingLengthPre-data.bathingLengthPre)
...
class form(Form):
    ...
    getupHour = IntegerField("今日の起床時間",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=24)])
    getupMin = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
    ...
    sleepMinEnd = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    goingoutHourPre = IntegerField("外出時間",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=12)])
    goingoutMinPre = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    ...
    goingoutMin = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    cookingHourPre  = IntegerField("料理時間",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=10)])
    cookingMinPre = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    ...
    cookingMin = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    eatingHourPre = IntegerField("食事時間",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=10)])
    eatingMinPre = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    ...
    eatingMin = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
    bathingHourPre  = IntegerField("入浴時間",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=10)])
    bathingMinPre = IntegerField("分",
                    [validators.InputRequired("この項目は入力必須です"),
                    validators.NumberRange(min=0, max=59)])
...
def root():
    idForm = form(request.form)
    data = Data()
    print(request.method)
    if request.method == 'POST':
        print(idForm.validate())
        if idForm.validate() == False:
            flash("全て入力する必要があります。")
            return render_template('index.html', idForm=idForm, data=data, pred=0, sendStatus=0)
        else:
            getSleepForm(data)
            if data.otherLength < 0 or data.otherLength < 0:
                flash("otherが負の値")
                print("負の値")
                return render_template('index.html', idForm=idForm, data=data, pred=0, sendStatus=0)
            if "numberTimes" in session:
                session["numberTimes"] = session["numberTimes"] + 1
            else:
                session.permanent = True
                session["numberTimes"] = 0
            print("numberTimes:",session["numberTimes"])

            pred,sendStatus = getForm(data.sleepingLength,data.goingoutLength,data.cookingLength,data.eatingLength,data.bathingLength,data.otherLength,session["numberTimes"])
            # if data.numberTimes == 0:
        #     saveFile(data.id, data.day, data.goingoutLength,data.cookingLength,data.eatingLength,data.bathingLength,data.otherLength, pred, 
#             data.sleepHourStart, data.sleepMinStart, data.sleepHourEnd, data.sleepMinEnd,data.sleepingLength, data.timeLeft,
#             data.goingoutLengthPre,data.cookingLengthPre,data.eatingLengthPre,data.bathingLengthPre,data.otherLengthPre,fileState=2)
            return render_template('index.html', idForm=idForm, data=data, pred=pred, sendStatus=sendStatus)
    else:
        return render_template('index.html', idForm=idForm, data=data, pred=0, sendStatus=0)

# @app.route('/end/<data>')# どのページで実行する関数か設定
# def end(data):
#     print(1)
#     idForm = form(request.form)
#     print("お疲れ様でした")
